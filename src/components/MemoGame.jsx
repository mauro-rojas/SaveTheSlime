...
const FOODS = [huevos, papa, pollo, queso, sushi, tocino].flatMap((food) => [
  `a|${food}`,
  `b|${food}`,
]);

const LICUADORAS = [licuadora].flatMap((licuadora) => [
  `a|${licuadora}`,
  `b|${licuadora}`,
  `c|${licuadora}`,
  `d|${licuadora}`,
...
const HACHUELAS = [hachuela].flatMap((hachuela) => [
  `a|${hachuela}`,
  `b|${hachuela}`,
  `c|${hachuela}`,
  `d|${hachuela}`,
...
const FICHAS = FOODS.concat(LICUADORAS.concat(HACHUELAS)).sort(
  () => Math.random() - 0.5
);
...
export default function MemoGame({
  ...
  useEffect(() => {
    if (guessed.length >= 12) {
      if (
        guessed.filter(
          (element) =>
            element.split("|")[1] !== licuadora &&
            element.split("|")[1] !== hachuela
        ).length === FOODS.length
      ) {
        setTimeout(() => {
          setWin(true);
  ...
  return (
    <motion.ul className={styles.fichasContainer}>
      {FICHAS.map((ficha, i) => {
        ...
        return (
          <motion.li
            ...
            onClick={() => {
              if (
                selected.length < 2 &&
                !selected.includes(ficha) &&
                !guessed.includes(ficha)
              )
